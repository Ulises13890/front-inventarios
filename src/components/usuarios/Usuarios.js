...
import { getUsuarios, postUsuarios, putUsuarios} from '../../services/UsuarioService'
...
setTipos(resp.data);
 ...
 const guardarTipoEquipo = async () => {
   try{
    const res = await postUsuarios(tipo1);
    console.log(res);
    setTipo({nombre: ''});
    ListTipoEquipos();
  }catch(error){
...
const editarTipoEquipo = async (e) => {
    try{
    const resp = await putUsuarios(tipo1._id, tipo1);
    console.log(resp)   
    ListTipoEquipos()
